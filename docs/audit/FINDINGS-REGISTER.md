...
## Decisions pending (owner input required)

1. **D.NN — User provisioning & onboarding model** (ARCH-4) — author draft, approve.
2. **SEC-1 — may a resident see their own un-masked national-ID?** (drives portal fix).
3. **ISO-SCOPE — obtain in-scope controls / SoA from ISO auditor** (unblocks wave 2).
