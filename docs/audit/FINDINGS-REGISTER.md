...
| SEC-7     | LOW                    | Contractor share-token reuses `JWT_SECRET` (audience `emapp-share` isolation is the live defence — a token signed for any other tier is rejected; residual: a `JWT_SECRET` leak compromises all HS256 tiers at once). Surfaced in #211 independent security review.            | Introduce a dedicated `SHARE_TOKEN_SECRET` (mirrors the signature-token secret separation).    | Token signed with another tier's secret/audience → rejected; boot requires the new secret. |
| SEC-8     | LOW                    | `ContractorAuthGuard` assigns `shares.permissions` (jsonb) via TS-cast — no runtime `z.parse`. Trusted-at-rest (managers write only via the `.strict()` `SharePermissionsSchema` DTO), but a future malformed-write path would bypass the structural perms gates. #211 review. | Pass `share.permissions` through the existing `resolveSharePermissions()` helper in the guard. | Malformed jsonb permissions → guard normalizes/rejects, not trusted blindly.               |
...
### D2 close-out — built + closed (audited 2026-06-01)

**D2 shipped — all 6 tiers complete:** Provider console (suspend/reactivate +
onboarding) · Agent capability UI · reveal-on-demand PII button · Resident portal
(D.47 masking + aggregate progress) · contractor share-default fix · **Contractor
read-tier (#211)**. #201–207 + #209–212, all audited (Gate-6 holds — no
policy/migration; security properties verified).

**Closed — built, verified, merged (2026-06-01):** the 3 gaps deferred at the
D2 close-out were then fixed and independently verified before merge.

| ID       | Sev      | Resolution                                                                                                                                                                                                                                                                                                | PR / verification                                                                                                  |
| -------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| D2-DEF-1 | MED      | **Contractor read-tier** — share-token (audience `emapp-share`, alg-pinned) scoped read-view: project + structural apartments + aggregate signatures + project-level shared docs; owners-PII OFF **structurally** (no owners table queried), IDOR-safe download, revocation + org-suspension per request. | **#211** — code + security reviewers PASS (0 CRIT/HIGH); 26 API tests + independent security re-review; no Gate-6. |
| D2-DEF-2 | LOW (UX) | **AccessReasonGate** now mirrors the BE `validateProviderReason` exactly (ticket-ref OR ≥20 substantive chars, ≤512). No input passes the gate but fails `withProvider`.                                                                                                                                  | **#209** — faithful mirror, 17 FE tests.                                                                           |
| D2-DEF-3 | LOW (UX) | **Reveal-PII button** gated on `view_owner_pii`, now carried on the `/me` profile (manager always / agent per-capability / viewer never). Resolved server-side per request; JWT unchanged; schema add-only.                                                                                               | **#210** — 5 BE tests (incl. agent-without-capability → hidden).                                                   |

**Perf-closure (#212, D.57):** the aggregate signature-progress query
(`(d.project_id = P OR b.project_id = P)`) had **no index path** — the OR
straddled two tables → seq-scan on `signature_requests` + `documents` at scale
(owner pain #1). Root-fixed `OR → UNION` + `archived_at IS NULL` (**active-doc
consent** — a signature on a superseded/archived agreement is not valid consent;
uniform portal + contractor). Uses the **existing partial indexes** (no new
index → **not Gate-6**). Enforced permanently by an EXPLAIN/index-path CI gate
(`signature-progress-perf.spec` — fails if the seq-scan shape returns). Decision
locked: **D.57**. The CI clean-DB run (not just local volume EXPLAIN) was what
proved the index path — recorded as the verification standard for perf fixes.
