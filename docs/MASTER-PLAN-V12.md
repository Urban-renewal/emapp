...
| ----- | ------------------------------------------------------------------------------------ | ------- | ------------------------------------------------- |
| A1    | **ENV-1**: fix turbo strict-env so `infisical run -- pnpm dev` boots a healthy stack | —       | fresh `pnpm dev` → API 200 + login works          |
...
| D0.5            | **admin-app scaffold (D.45)**: second Cloudflare Pages app on `admin.emapp.io` + provider cookie scope isolation                                                                                                                                                     | DEC-1   | provider login on admin subdomain; cookie not shared with customer app                                               |
| D1              | **ARCH-2**: Provider→Org onboarding (create org + first-manager invite)                                                                                                                                                                                              | D0.5    | provider-creates-org E2E → manager invite → password → login own tenant                                              |
...
| Slice | Goal                                                                | Verification                   |
| ----- | ------------------------------------------------------------------- | ------------------------------ |
| PL1   | **PERF-6**: `next build && next start` + colocated DB pass          | production-absolute ms numbers |
