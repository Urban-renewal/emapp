/**
 * Notes query-key builders — a PLAIN module (NO `'use client'`) so BOTH
 * the client hook (`use-notes.ts`) and the server RSC prefetch
 * (`notes/page.tsx`) can import AND call it. The builder previously lived
 * inline in `use-notes.ts`, but that file is `'use client'`, so a Server
 * Component calling `notesListQueryKey()` would hit Next's client/server
...
/**
 * Shape is `['notes', 'list', query, locale]` — `query` is the LITERAL
 ...
 */
export function notesListQueryKey(query: { limit?: number; cursor?: string }, locale: 'he' | 'en') {
  return [...NOTES_KEY, 'list', query, locale] as const;
}
