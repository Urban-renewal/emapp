...
- **Tracked debt from the audit (NOT merge-blockers; recommended before "enterprise/ISO-ready"):** (a) D.17 deny-matrix for viewer/agent is un-black-box-testable until an invite/membership endpoint exists — biggest ISO-A.9 gap, enforced in-code+reviewed only; (b) duplicated security-critical code across ~13 services (`isUniqueViolation` ×5, `requireManager`, `assertXVisible`) → extract one audited module (single source for tenant/role/pagination/pg-error mapping); (c) audit lacks actor IP/User-Agent (ISO A.12.4) — AuditService supports it, services don't pass req context; (d) no Idempotency-Key on creates (docs/09 §3.10 specifies it) — double-submit → duplicate rows; (e) nested GET/list does 2 sequential queries (assertXVisible + data) → fold into one for latency; (f) no `SET LOCAL statement_timeout` in withTenant.
