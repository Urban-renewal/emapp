...
- **Track D — slice (D.54 close-out 2/3: narrow assignee tasks.update, auto-merge).**
  ...
  - Evidence: `tasks-capability.spec` TASK-A1 (assignee no-cap → set status), A2
    ...
    - A1..7 green; agent-capabilities + documents-capability green (refactor safe).
      lint+typecheck green. No policy/migration → auto-merge.
  - **MERGED** as #195 (both reviewers PASS, CI green, auto-merged).

- **Track D — slice (D.54 close-out 3/3: fail-open CI guard, auto-merge).** Turns
  the per-cell "GUARDRAIL: every write endpoint MUST call requireAgentCapability"
  comments in `policy.ts` into a mechanical wall (like the tenant-isolation guard).
  - `architecture/agent-capability-guard.ts` (+ `.spec.ts`) — source-text scanner:
    for every controller endpoint whose (resource, action) cell coarsely admits
    `agent` on a WRITE action (create/update/delete, via `@AuthzResource` + HTTP
    verb / `@AuthzAction` override, matched against the live `POLICY`), it resolves
    the delegated `this.x.<method>` and FAILS if NO delegated service method calls
    `requireAgentCapability`/`agentHasCapability`. A reviewed ALLOWLIST carries the
    5 deliberate non-capability exceptions: notes create/update/archive (D.17
    authorship) + notifications markRead/markAllRead (RLS self-scope).
  - Verified MEANINGFUL (not vacuous): with the allowlist emptied the guard flags
    EXACTLY those 5 and nothing else — proving every D.46 capability endpoint
    (buildings/apartments/owners-update+delete/tasks/documents/signatures/imports)
    is recognised as gated. 4 spec tests: sanity (≥15 endpoints), no-ungated,
    positive gate-detection (every capability resource has a gated endpoint),
    no-stale-allowlist. Runs in CI via the standard `*.spec.ts` vitest pattern.
    lint+typecheck green. No policy/migration → auto-merge. **Completes D1.**
