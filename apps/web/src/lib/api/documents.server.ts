/**
 * Server-only Documents API — the RSC-prefetch twin of `lib/api/documents.ts`
 * (perf-research/01-rsc-waterfall.md §2.4 / §4.2).
 *
 * Lives in a SEPARATE module (not in `documents.ts`) on purpose: `documents.ts`
 * is imported by `'use client'` pages/hooks, and pulling `next/headers` (via
 ...
 * prefetch still runs the IDENTICAL defensive Zod parse (`DocumentSchema` +
 * `PageSchema`) so the dehydrated cache entry is byte-identical to what the
 * client `listDocuments` queryFn produces — the hook's `select` adapter then
...
import type { DocumentListPage } from './documents';
import { ApiClientError } from './errors';
import { PageSchema } from './paging';

/**
 * Server-side `GET /api/v1/documents` for the page prefetch. Forwards the
 ...
 */
export async function serverListDocuments(query: {
  limit?: number;
  cursor?: string;
  projectId?: string;
  apartmentId?: string;
  ...
  const body = await serverApiGet(`/documents${qs ? `?${qs}` : ''}`);
  ...
  // SAME parse as the client `listDocuments` — the wire is the source of truth.
  const items = z.array(DocumentSchema).parse(data);
