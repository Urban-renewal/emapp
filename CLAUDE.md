...
\- Auth: Better Auth; MFA mandatory for Provider Admin

\- Cache: PostgresCacheProvider (cache\_kv) in MVP — no Redis
...
\- National ID field = national\_id (NOT tz). PII. (Decision D.19)

\- Project status enum: planning | gathering\_signatures | approved |

&#x20; in\_construction | completed | cancelled. (Decision D.18)

\- PII (national\_id, phone, signatures) encrypted via pgcrypto.

&#x20; Never logged, never in error messages.

\- Hebrew names sort with COLLATE he\_il\_icu.
...
1\. Read docs/DECISIONS.html (D.01-D.20 are law — national\_id not tz, {data} envelope, status enum, 6 roles, /api/v1/). Then read the task from its source doc:
...
&#x20; c) update PROGRESS.md (phase checkbox, status: awaiting\_approval)
