 ...
 *
 * The guarantee under test: production must FAIL FAST if real creds are absent
 * (NoopSMSProvider would make Tenant OTP + signature SMS silently undeliverable),
 * while dev/test/unset falls back to Noop, and ALL THREE creds are required.
 */
...
describe('smsProviderFactory — non-production fallback', () => {
  it('development + no creds → NoopSMSProvider', () => {
    process.env['NODE_ENV'] = 'development';
    expect(smsProviderFactory()).toBeInstanceOf(NoopSMSProvider);
  });

  it('NODE_ENV unset + no creds → NoopSMSProvider (does NOT throw)', () => {
    expect(smsProviderFactory()).toBeInstanceOf(NoopSMSProvider);
  });

  it('test env + partial creds → NoopSMSProvider (no throw outside production)', () => {
