/**
 * Server-only Notes API — the RSC-prefetch twin of `lib/api/notes.ts`
 * (perf-research/01-rsc-waterfall.md §2.4 / §4.2).
 *
 * Lives in a SEPARATE module (not in `notes.ts`) on purpose: `notes.ts` is
 ...
 * the IDENTICAL defensive Zod parse (`NoteSchema` + `PageSchema`) so the
 * dehydrated cache entry is byte-identical to what the client `listNotes`
...
import type { NoteListPage } from './notes';
import { PageSchema } from './paging';

/**
 * Server-side `GET /api/v1/notes` for the page prefetch. Forwards the
 ...
 */
export async function serverListNotes(query: {
  limit?: number;
  cursor?: string;
  ...
  const body = await serverApiGet(`/notes${qs ? `?${qs}` : ''}`);
  ...
  // SAME parse as the client `listNotes` — the wire is the source of truth.
  const items = z.array(NoteSchema).parse(data);
